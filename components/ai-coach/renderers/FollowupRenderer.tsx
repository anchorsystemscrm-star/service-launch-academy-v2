  ...
  return (
    <div className="grid gap-4">
      <article className="rounded-[24px] border border-white/10 bg-black/20 p-5">
        ...
        <h3 className="mt-3 text-xl font-semibold text-white">{data.goal}</h3>
      </article>

      <div className="grid gap-4">
        {data.sequence.map((item) => (
          <article key={`${item.day}-${item.channel}-${item.message}`} className="rounded-[24px] border border-white/10 bg-black/20 p-5">
            ...
            <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-100">{item.message}</p>
          </article>
        ))}
      </div>
    </div>
