...
import { getPricingHref, normalizeSubscriptionTier, tierDescriptions, tierLabels } from "@/utils/access";
import { useAccessProfile } from "@/utils/storage";
...
export default function PricingPage() {
  ...
  const focusedPlan = useMemo(
    () => normalizeSubscriptionTier(searchParams.get("plan")),
    [searchParams]
  );
    ...
    <div className="mx-auto max-w-7xl animate-fade-up">
          ...
          <div className="mt-8 grid gap-8 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="rounded-[28px] border border-white/10 bg-white/5 p-6">
              ...
              <div className="mt-4 flex items-center justify-between gap-4">
                <div>
                  <p className="text-2xl font-semibold text-white">{tierLabels[profile.tier]}</p>
      ...
      <section className="mt-6 grid gap-5 xl:grid-cols-4">
        {planContent.map((item) => {
          const current = profile.tier === item.plan;
